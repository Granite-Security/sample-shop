 ...
 *
 * The switch is on `confirmationMode`, not on provider id, which is what keeps the
 * frontend additive: most non-card providers are redirect-shaped and need only a
 * selector entry and a backend adapter. Only a genuinely new *shape* of
 * confirmation — a second in-page SDK — needs a component here.
 */
export default function PaymentWidget({
  ...
  // this build does not know about. That is deploy skew, and throwing would blank
  // the checkout page instead of showing the shopper what went wrong.
  return (
    <p className="border-l-2 border-terracotta bg-terracotta/10 px-4 py-3 text-sm text-terracotta">
      This build cannot complete payments with {displayName ?? provider}. Please choose another
      payment method.
    </p>
  );
}
