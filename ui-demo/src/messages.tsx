...
/**
 * Unread-message count for the header bell. Mirrors
 * ui-shop/src/contexts/MessagesContext.tsx — same endpoint, same polling rules.
 *
 * Polls rather than streaming (docs/users/messaging.md D7): one indexed COUNT
 * per signed-in user every 30s, against an inbox nobody expects to be
 * sub-second.
 */
export function MessagesProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    // Coming back to the tab is exactly when a stale badge is most obvious.
    const onVisible = () => {
      if (document.visibilityState === 'visible') refresh();
    };
